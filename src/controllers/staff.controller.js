...
exports.create = function (req, res) {
    ...
    if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
        ...
        Staff.create(new_staff, function (err, staff) {
            if (err)
                res.send(err);
            res.json({ error: false, message: "staff added successfully!", data: staff });
        });
    }
};
...
exports.update = function (req, res) {
    if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
        ...
        Staff.update(req.params.id, new Staff(req.body), function (err, staff) {
            if (err)
                res.send(err);
            res.json({ error: false, message: 'staff successfully updated' });
        });
    }
...
exports.delete = function (req, res) {
    Staff.delete(req.params.id, function (err, staff) {
        if (err)
            res.send(err);
        res.json({ error: false, message: 'staff successfully deleted' });
    });
};